...
const installerName = 'Simple Download Manager_0.3.46-alpha_x64-setup.exe';
assert.equal(
  githubReleaseAssetName(installerName),
  'Simple.Download.Manager_0.3.46-alpha_x64-setup.exe',
);
assert.equal(
  updaterAssetUrl(
  ...
  'https://github.com/JustNak/SimpleDownloadManager/releases/download/updater-alpha/Simple.Download.Manager_0.3.46-alpha_x64-setup.exe',
);

const latest = createLatestAlphaJson({
  version: '0.3.46-alpha',
  notes: 'Alpha update',
...
assert.equal(latest.version, '0.3.46-alpha');
...
assert.match(latest.platforms['windows-x86_64'].url, /Simple\.Download\.Manager_0\.3\.46-alpha_x64-setup\.exe$/);
