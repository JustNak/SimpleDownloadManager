...
assert.match(
  defaultsSource,
  /bulk:\s*\{[\s\S]*outputDirectory:\s*defaultBulkDownloadDirectory\(downloadDirectory\)[\s\S]*maxConcurrentDownloads:\s*4[\s\S]*speedLimitKibPerSecond:\s*0[\s\S]*downloadPerformanceMode:\s*'balanced'[\s\S]*hosterFairnessMode:\s*'adaptive'[\s\S]*hosterAccelerationMode:\s*'safe'[\s\S]*autoRetryOverrideEnabled:\s*false[\s\S]*autoRetryAttempts:\s*3[\s\S]*startBehavior:\s*'review_then_start'[\s\S]*expandActiveRowsByDefault:\s*false/,
