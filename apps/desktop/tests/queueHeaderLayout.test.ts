...
assert.match(fileBadgeSource, /function fileBadgeDensity\(rowSize: QueueRowSize/, 'file badges should keep Svelte density sizing');
...
assert.match(source, /detailsHeight/, 'details pane should keep the Svelte resizable height state');
...
assert.match(queueLayoutSource, /function detailsLevelForHeight\(height: number\): DetailsLevel/, 'details pane should expose compact, standard, and expanded height levels');
assert.match(source, /GripHorizontal/, 'details pane should keep the Svelte resize affordance');
...
assert.match(source, /function toggleSingleJobSelection\(jobId: string\)/, 'queue row click should keep the Svelte toggle selection helper');
assert.match(source, /function applySelectionRange/, 'queue should keep Svelte drag/range selection behavior');
...
assert.match(queueLayoutSource, /function queueHeaderSelfClass\(align: QueueTableAlignment\)[\s\S]*justify-self-start/, 'sortable queue headers should use grid self-alignment instead of stretching across the whole column');
...
assert.match(queueLayoutSource, /const QUEUE_TABLE_GRID_CLASS = 'grid-cols-\[minmax\(420px,2\.8fr\)_150px_110px_100px_150px_72px\]'/, 'queue table should centralize the shared header and row grid template');
assert.match(queueLayoutSource, /type QueueTableAlignment = 'start' \| 'center' \| 'end'/, 'queue table should expose explicit alignment modes for header and data cells');
assert.match(queueLayoutSource, /function queueHeaderCellClass\(align: QueueTableAlignment = 'start'\)[\s\S]*flex min-w-0 items-center px-1\.5/, 'queue headers should use the same inner padding as their data cells');
assert.match(queueLayoutSource, /function queueTableCellClass\(align: QueueTableAlignment = 'start'\)[\s\S]*flex min-w-0 items-center px-1\.5[\s\S]*tabular-nums text-muted-foreground/, 'queue data cells should share the header alignment and tabular numeric styling');
...
assert.match(queueLayoutSource, /function queueDateCellClass\(\)[\s\S]*return queueTableCellClass\('center'\)/, 'date values should be centered under the date header');
assert.match(queueLayoutSource, /function queueMetricCellClass\(\)[\s\S]*return queueTableCellClass\('center'\)/, 'speed or seed values should be centered under their header');
