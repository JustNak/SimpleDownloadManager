...
struct PreparedTorrentAddRequest<'a> {
    state: &'a SharedState,
    job_id: &'a str,
...
fn validate_zip_entry_name(name: &str) -> Result<String, String> {
...
fn write_zip_local_header(writer: &mut impl Write, name: &str) -> Result<(), String> {
...
fn write_zip_file_data(writer: &mut impl Write, source_path: &Path) -> Result<u32, String> {
...
fn write_zip_data_descriptor(writer: &mut impl Write, crc32: u32, size: u32) -> Result<(), String> {
...
fn write_zip_central_directory_entry(
...
fn zip_entry_name_bytes(name: &str) -> Result<&[u8], String> {
    let bytes = name.as_bytes();
    checked_u16_len(bytes.len(), "archive entry name")?;
    Ok(bytes)
}

fn checked_u16_len(value: usize, label: &str) -> Result<u16, String> {
    u16::try_from(value).map_err(|_| format!("{label} exceeds the ZIP32 limit."))
}

fn checked_u32_position(position: u64) -> Result<u32, String> {
    u32::try_from(position).map_err(|_| "Archive exceeds the ZIP32 size limit.".into())
}

fn zip_stream_position(writer: &mut impl Seek) -> Result<u64, String> {
    writer
        .stream_position()
        .map_err(|error| format!("Could not read ZIP writer position: {error}"))
}

fn write_u16_le(writer: &mut impl Write, value: u16) -> Result<(), String> {
    writer
        .write_all(&value.to_le_bytes())
        .map_err(|error| format!("Could not write ZIP archive: {error}"))
}

fn write_u32_le(writer: &mut impl Write, value: u32) -> Result<(), String> {
    writer
        .write_all(&value.to_le_bytes())
        .map_err(|error| format!("Could not write ZIP archive: {error}"))
}
...
const ZIP_GENERAL_PURPOSE_FLAGS: u16 = 0x0808;
    ...
    fn zip_central_directory_names(path: &Path) -> Vec<String> {
        ...
        let entry_count =
            u16::from_le_bytes(bytes[eocd_index + 10..eocd_index + 12].try_into().unwrap())
                as usize;
        let mut offset =
            u32::from_le_bytes(bytes[eocd_index + 16..eocd_index + 20].try_into().unwrap())
                as usize;
