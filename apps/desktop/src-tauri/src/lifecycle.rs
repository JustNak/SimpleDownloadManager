...
pub fn handle_run_event(event: RunEvent) {
    if let RunEvent::ExitRequested { code, api, .. } = event {
        if should_prevent_exit_request(code) {
            api.prevent_exit();
        }
    }
}

pub fn should_prevent_exit_request(exit_code: Option<i32>) -> bool {
    exit_code.is_none()
}
...
pub fn ensure_main_window<R: Runtime>(
    ...
    let window = WebviewWindowBuilder::new(
        app,
        config.label,
        WebviewUrl::App(main_window_url(selected_job_id).into()),
    )
    .title(config.title)
...
fn main_window_url(selected_job_id: Option<&str>) -> String {
    let Some(job_id) = selected_job_id else {
        return "index.html".into();
    };

    let mut query = url::form_urlencoded::Serializer::new(String::new());
    query.append_pair("selectJob", job_id);
...
mod tests {
    ...
    #[test]
    fn implicit_last_window_exit_requests_are_prevented_for_tray_persistence() {
        assert!(super::should_prevent_exit_request(None));
    }

    #[test]
    fn explicit_exit_requests_are_allowed() {
        assert!(!super::should_prevent_exit_request(Some(0)));
        assert!(!super::should_prevent_exit_request(Some(1)));
    }

    #[test]
    fn selected_job_main_window_url_encodes_query_value() {
        assert_eq!(
            super::main_window_url(Some("job:one/two three")),
            "index.html?selectJob=job%3Aone%2Ftwo+three"
        );
        assert_eq!(super::main_window_url(None), "index.html");
    }
