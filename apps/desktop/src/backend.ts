...
export async function clearTorrentSessionCache(): Promise<TorrentSessionCacheClearResult> {
  if (!isTauriRuntime()) return (await loadBackendMock()).clearTorrentSessionCache();
...
export async function takePendingSelectedJobRequest(): Promise<string | null> {
  if (!isTauriRuntime()) return (await loadBackendMock()).takePendingSelectedJobRequest();
  return invokeCommand<string | null>('take_pending_selected_job_request');
}
