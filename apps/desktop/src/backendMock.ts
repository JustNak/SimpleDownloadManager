...
import {
  ...
  type DownloadPrompt,
  type Settings,
  type TorrentSessionCacheClearResult,
} from './types';
...
export async function resumeAllJobs(): Promise<void> {
  ...
  mockState.jobs = mockState.jobs.map((job) =>
    resumableStates.includes(job.state) ? resetForRetry(job) : job,
  );
  emitMockState();
}
...
export async function confirmDownloadPrompt(
  ...
  _options: ConfirmDownloadPromptOptions = {},
): Promise<void> {}
...
export async function takePendingSelectedJobRequest(): Promise<string | null> {
  return null;
}
...
export async function openProgressWindow(id: string): Promise<void> {
  ...
  window.open(
    popupUrl(`?window=download-progress&jobId=${encodeURIComponent(id)}`),
