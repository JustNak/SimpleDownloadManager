...
- Alpha package versions must use a numeric `version` such as `0.3.41` and a human-readable `version_name` such as `0.3.41-alpha`.
- Keep `browser_specific_settings.gecko.strict_min_version` aligned with the AMO lint target used by the release scripts.
- Declare `data_collection_permissions` when download URLs, referrer/page metadata, response headers, filename hints, content length, or user download actions are sent to the local native app.
...
- `webRequest` and `webRequestBlocking` are required so Firefox can intercept attachment/download responses before the default Save As dialog opens.
...
- Reviewer notes should explain the native app dependency, download handoff flow, fallback behavior, wildcard excluded hosts, and why each permission is needed.
